...
\begin{itemize}
	\item\textit{Graph Connectivity.} Given a graph $G=(V,E)$, is it
	      connected?
	      \begin{align*}
		      L & =\{x\in\binrep\mid x\text{ encodes a connected graph}\} \\
	...
	\item\textit{Max Flow (Decision Version).} Given a flow network
	      ...
	      value $\geq k$?
	      \begin{align*}
		      L & =\{x\in\binrep\mid x\text{ encodes a $(G,k)$ such that $\text{val}(G)\geq k$}\} \\
		        & =\{(G,k)\mid\text{val}(G)\geq k \}
	      \end{align*}
	\item\textit{Sum.} Given $a,b,c\in\Z$, does $a+b=c$?
	      \begin{align*}
		      L & =\{x\in\binrep\mid x\text{ encodes $(a,b,c)$ such that $a+b=c$}\} \\
		        & =\{(a,b,c)\mid a+b=c\}
	      \end{align*}
\end{itemize}
