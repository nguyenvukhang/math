...
In $\R^n$ with the standard inner product (dot product), the
...
where the sums run for $i$ in $\{\iter1n\}$.

\begin{compute}
	Let $u,v\in\R^n$. Let $w_i$ be the $i^\text{th}$ component of vector
	$w$.

	Then by definition of the dot proudct, we have
	$$
		|\inner uv|^2=|u_1v_1+u_2v_2+\ldots+u_nv_n|^2
	$$

	and
	$$
    \inner uu\cdot\inner vv=(u_1^2+\ldots+u_n^2)(v_1^2+\ldots+v_n^2)
	$$

  which gives us the Cauchy-Schwarz inequality for $\R^n$.
\end{compute}
