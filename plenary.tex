...
if for every $\epsilon>0$ there exists a point $x\in A\setminus\{c\}$ such that
$x\in(c-\epsilon,c+\epsilon)$.

In complex analysis, $c\in\C$ is a cluster point of a non-empty set
$A\subseteq\C$ if for every $\epsilon>0$ there exists a point $z\in
A\setminus\{c\}$ such that $z\in B_\epsilon(c)$.
