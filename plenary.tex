...
\Theorem{1.1.5}{Monotone sequence with a convergent subsequence is convergent}\label{aaf3ba6}
...
\Definition{1.1.13}{Norm properties}\label{e0fff96}
...
\Definition{9.1.2}{Turing reduction}\label{e8ff6a1}

Reducing $A$ to $B$ by Turing reduction is to find a function which
mimics the behavior of $A$ using an oracle of $B$.

\Definition{9.1.2}{Many-one reduction}\label{e009acb}

Reducing $A$ to $B$ by Many-one reduction is to find a function $f$
which converts inputs $x$ of $A$ into inputs $f(x)$ of $B$, such that
$A(x)=B(f(x))$ under all values of $x$.

That is, $A(x)=\text{TRUE}\iff B(f(x))=\text{TRUE}$.

\Definition{9.1.3}{Everything $\mathsf P$-, $\mathsf{NP}$-related}\label{c8c65c8}
...
Given problems $A,B$, we say that $A\leq_{\mathsf P}B$ if there exists
a Turing Reduction from $A$ to $B$, or a Many-One Reduction from $A$
to $B$.

An algorithm $A$ runs in \textit{polynomial time} if $\exists c\in\R$
s.t. for all binary inputs $x$, $A$ terminates after $O(|x|^c)$
computation steps.

A decision problem $L$ is \textit{polynomial-time computable} if there
exists a polynomial-time $A$ s.t. $\forall x$, $x\in L\iff
	A(x)=\text{TRUE}$.

We define
$$
	\mathsf P:=\{L\mid L\text{ is polynomial-time computable}\}
$$
