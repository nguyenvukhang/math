\section{Ordinary Differential Equations}\label{e598fa6}

\Subsection{First order differential equations}\label{e55966b}

\Result{1.1.1}{Separable}\label{c4db619}
...
\Result{1.1.2}{Homogeneous}\label{bb1ee90}
...
\Result{1.1.4}{Bernoulli}\label{dc54d9b}
...
\Result{1.1.5}{Riccati}\label{e5b78d0}
...
\Result{1.1.6}{Exact}\label{db2397c}
...
\Proposition{3.1.5}{Power series at zero}\label{fe2e0f1}
...
\Definition{3.1.6}{Laplace transform}\label{a7047c9}
...
\Definition{4.0.0}{Regular singular points}\label{f490530}
...
\Remark{4.0.1}{Series solutions}\label{fdab69a}
...
\Remark{4.0.2}{Frobenius series}\label{b1a3270}
...
\Definition{5.0.0}{Dirac delta function}\label{a2fd0bb}
...
% \begin{align*}
%    &y''+P(x)y'+Q(x)y                                       \\
%    &=\bigg[\sum_{n=0}^\infty(r+n)(r+n-1)a_nx^{r+n-2}\bigg]
%   +P(x)\bigg[\sum_{n=0}^\infty(r+n)a_nx^{r+n-1}\bigg]
%   +Q(x)\bigg[\sum_{n=0}^\infty a_nx^n\bigg]                \\
%    &=\bigg[
