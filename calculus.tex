...
\Definition{1.1.6}{Directional derivative}\label{dbbf530}
...
\Remark{1.1.7}{Gradient}\label{dad2713}

Only scalar-valued functions can have gradients.

The \textbf{gradient} of a function $f:\R^n\to\R$ is the vector-valued
function $\nabla f$ whose value at $p$ gives the direction and rate of
fastest increase. Further, $\nabla f$ can be written as
$$
	\nabla f(p)=\begin{mat}
...
product with any vector $d$ at each point $x$ is the directional
derivative of $f$ along $d$. That is,
$$
	\nabla f(x)^Td=\href{dbbf530}{f'(x;d)}
$$

\Remark{1.1.8}{Jacobian}\label{a2ceb99}

The Jacobian of a vector-valued function in several variables
generalizes the \href{dad2713}{gradient} of a scalar-valued function
in several variables.

In other word, the Jacobian of a scalar-valued function is its
gradient.

So let $f:\R^n\to\R^m$. In particular,
$$
	f(x):=\begin{mat}f_1(x)\\\vdots\\f_m(x)\end{mat}\with f_i:\R^n\to\R
$$
...
$$
	...
	\def\n#1{\nabla^Tf_{#1}}
	\mathbf{J}
	=\begin{mat}\p f{x_1}&\ldots&\p f{x_n}\end{mat}
	=\begin{mat}\n1\\\vdots\\\\\n m\end{mat}
	=\begin{mat}
