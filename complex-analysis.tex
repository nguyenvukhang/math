...
\Theorem{0.0.2}{Complex differentiability}\label{d1d5d93}
...
$x\in(\alpha,\beta)\setminus\{p\}$. Then $(dH/dx)(p)$ exists and
$(dH/dx)(x)=F(x)$ for all $x\in(\alpha,\beta)$.
$$
...
continuously differentiable on $U\setminus\{P\}$. Suppose further that
$$
...
fixed. Suppose that $F$ is continuous on $U$ and holomorphic on
$U\setminus\{P\}$. Then there is a holomorphic $H$ on $U$ such that $\partial
H/\partial z=F$.
...
If $f:\C\setminus\{0\}\to\C$ is a holomorphic function, and if $\gamma_r$
describes the circle of radius $r$ around $0$, tranversed once around
counterclockwise, then, for any two positive numbers $r_1<r_2$,
$$
...
for any piecewise $C^1$ closed curve $\gamma$ in $U\setminus\{z\}$ that can be
continuously deformed in $U\setminus\{z\}$ to $\partial D(z,r)$ equipped with
...
sequence $\{z_j\}\subseteq\mathbf Z\setminus\{z_0\}$ such that $z_j\to z_0$,
then $f\equiv0$.
...
Note the strength of this statement. As long as $f$ is holomorphic, if it's zero
on just a tiny $D(P,r)$, then it is zero on the entire domain.

\Corollary{3.6.3}{}\label{ac6f6ea}

Let $U\subseteq\C$ be connected and open. Let $f,g$ be holomorphic on $U$. If
$\Set{z\in U}{f(z)=g(z)}$ has an \href{b0219cd}{accumulation point} in $U$, then
$f\equiv g$.

\Corollary{3.6.4}{}

Let $U\subseteq\C$ be connected and open and let $f,g$ be holomorphic on $U$. If
$f\cdot g\equiv0$ on $U$, then either $f\equiv0$ on $U$ or $g\equiv0$ on $U$.
